...
local indicatorRenderer = ISUIElement:new(0, 0, 0, 0)
...
local function renderIndicatorOverlay()
    ...
    for zombie in pairs(trackedHostiles) do
        if zombie and not zombie:isDead() then
            ...
            indicatorRenderer:drawRectStatic(drawX, drawY, size, size, 0.85, 1, 0, 0)
            indicatorRenderer:drawRectStatic(drawX, drawY, size, 1, 1, 0.35, 0, 0)
            indicatorRenderer:drawRectStatic(drawX, drawY + size - 1, size, 1, 1, 0.35, 0, 0)
            indicatorRenderer:drawRectStatic(drawX, drawY, 1, size, 1, 0.35, 0, 0)
            indicatorRenderer:drawRectStatic(drawX + size - 1, drawY, 1, size, 1, 0.35, 0, 0)
        end
    end
end
Events.OnPostUIDraw.Add(renderIndicatorOverlay)
